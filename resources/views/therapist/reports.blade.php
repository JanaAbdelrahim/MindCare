        ...
        <div class="container">
            <div class="d-flex flex-column flex-lg-row justify-content-between gap-3 align-items-lg-end mb-4">
                <div>
                    ...
                    <p class="work-subtitle">Create clinical summaries from intake scores and keep patient progress
                        organized.</p>
                </div>
            ...
            <div class="row g-4 mb-4">
                ...
                <div class="col-md-4">
                    <div class="soft-card metric-card">
                        ...
                        <div class="metric">{{ $reports->whereIn('condition_level', ['high', 'severe'])->count() }}
                        </div>
                    ...
                    <section class="soft-card">
                        ...
                        <form method="POST" action="{{ route('therapist.reports.store') }}">
                                ...
                                <select class="form-select" id="intake_form_id" name="intake_form_id" required>
                                    ...
                                    @foreach ($intakeForms as $intakeForm)
                                        <option value="{{ $intakeForm->id }}" @selected(old('intake_form_id') == $intakeForm->id)>
                                            {{ $intakeForm->patient?->first_name }}
                                            {{ $intakeForm->patient?->last_name }}
                            ...
                            <div class="mb-3">
                                ...
                                <input class="form-control" id="total_score" name="total_score" type="number"
                                    min="0" value="{{ old('total_score', 0) }}" required>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="condition_level" name="condition_level" required>
                                    @foreach (['low', 'medium', 'high', 'severe'] as $level)
                                        <option value="{{ $level }}" @selected(old('condition_level') === $level)>
                            ...
                            <div class="mb-3">
                                <label class="form-label" for="recommended_specialization">Recommended
                                    specialization</label>
                                <input class="form-control" id="recommended_specialization"
                                    name="recommended_specialization" value="{{ old('recommended_specialization') }}"
                                    placeholder="Anxiety, trauma, CBT...">
                            </div>

                            <div class="mb-3">
                                <label class="form-label" for="notes">Notes</label>
                                <textarea class="form-control" id="notes" name="notes" rows="5"
                                    placeholder="Clinical notes and care recommendation">{{ old('notes') }}</textarea>
                            </div>
                            ...
                            <table class="table mindcare-table">
                                ...
                                <tbody>
                                    @forelse ($reports as $report)
                                        <tr>
                                            <td>{{ $report->patient?->first_name }} {{ $report->patient?->last_name }}
                                            </td>
                                            ...
                                            <td>
                                                <div class="d-flex gap-2 flex-wrap">
                                                    <a class="mindcare-btn light"
                                                       href="{{ route('therapist.reports.show', $report) }}">View</a>
                                                    <a class="mindcare-btn"
                                                       href="{{ route('therapist.reports.pdf', $report) }}">PDF</a>
                                                </div>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="6">
                                                <div class="empty-state">
                                                    ...
                                                    <p>Create the first report after reviewing a patient's intake form.
                                                    </p>
                                                </div>
                                            </td>
                                        </tr>
